...
/// This class extends [AnimationComponent] and describes a HappyBird.
...
class Bird extends AnimationComponent {
  ...
  /// gravity of the bird = falling speed
  static const double GRAVITY = 1000;
  // --------
  // SETTINGS

  /// callback when the bird hits an object in [collides(Rect object)]
  Function onCollide;
  /// callback when the bird hits the ground
  Function onHitGround;
  /// callback when the bird hits the top
  Function onHitTop;
  ...
  final double _size;
  final HappyBirdGame _game;
  /// actual speed of the bird
  double _speedY = 0.0;


  /// Initialize the class with the given [_size] and [_game].
  Bird(this._game, this._size) : super.empty() {
    // size
    height = _size;
    width = _size;
    ...
    _idle = spriteSheet.createAnimation(0, from: 0, to: 2, stepTime: 0.2);

    // up animation
    _up = spriteSheet.createAnimation(0, from: 3, to: 5, stepTime: 0.2);
    // fall animation
    _fall = spriteSheet.createAnimation(0, from: 6, to: 8, stepTime: 0.2);
  ...
  /// This method checks if the bird hits the ground.
  ///
  ...
  bool isHittingGround() {
    if (y > _game.screenSize.height - _size) {
      ...
      // remove the speed
      _speedY = 0.0;/// This method checks if the bird hits the ground.
      // callback
      onHitGround?.call();
      return true;
    }
  ...
  /// This method checks if the bird hits the top.
  ///
  ...
  /// This method returns the rectangle of the bird.
  ///
  /// return
  ///   [Rect] of the hitbox.
  Rect toRect() {
